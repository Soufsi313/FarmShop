...
<div class="admin-header">
    <div class="d-flex justify-content-between align-items-center">
        <div>
            <h1><i class="fas fa-edit me-2"></i>Modifier la Catégorie</h1>
                    ...
                    <div class="row mb-4">
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('type') is-invalid @enderror" id="type" name="type" required>
                                ...
                                <option value="purchase" {{ old('type', $category->type) == 'purchase' ? 'selected' : '' }}>Achat</option>
                                <option value="rental" {{ old('type', $category->type) == 'rental' ? 'selected' : '' }}>Location</option>
                                <option value="both" {{ old('type', $category->type) == 'both' ? 'selected' : '' }}>Achat et Location</option>
                            </select>
                            @error('type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select @error('food_type') is-invalid @enderror" id="food_type" name="food_type">
                                <option value="non_food" {{ old('food_type', $category->food_type) == 'non_food' ? 'selected' : '' }}>Non alimentaire</option>
                                <option value="perishable" {{ old('food_type', $category->food_type) == 'perishable' ? 'selected' : '' }}>Périssable</option>
                                <option value="non_perishable" {{ old('food_type', $category->food_type) == 'non_perishable' ? 'selected' : '' }}>Non périssable</option>
                            </select>
                            @error('food_type')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        ...
                        <div class="col-md-4">
                            <div class="form-check mt-4">
                                ...
                                <input class="form-check-input @error('allows_returns') is-invalid @enderror" 
                                       ...
                                       name="allows_returns" 
                                       value="1" 
                                       {{ old('allows_returns', $category->allows_returns) ? 'checked' : '' }}>
                ...
                <div class="row text-center">
                    <div class="col-6">
                        <div class="border-end">
                            <h4 class="text-primary mb-0">{{ $category->products_count ?? 0 }}</h4>
                            <small class="text-muted">Produits</small>
                        </div>
                    </div>
                    <div class="col-6">
                        <h4 class="text-success mb-0">{{ $category->is_active ? 'Actif' : 'Inactif' }}</h4>
        ...
        <div class="card">
            ...
            <div class="card-body">
                ...
                <div class="alert alert-warning mt-3">
                    ...
                    <strong>Attention :</strong> Modifier le type de catégorie peut affecter l'affichage des produits associés.
                </div>
            </div>
        </div>
    </div>
