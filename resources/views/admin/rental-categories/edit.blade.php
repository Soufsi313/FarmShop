...
@section('title', 'Modifier Catégorie de Location - Dashboard Admin')
@section('page-title', 'Modifier la Catégorie de Location')
...
<div class="space-y-6">
    ...
    <div class="flex justify-between items-start">
        <div>
            <h2 class="text-2xl font-bold text-gray-900">Modifier {{ $rentalCategory->name }}</h2>
            <p class="text-gray-600">{{ $rentalCategory->slug }}</p>
        </div>
        <div class="flex space-x-3">
            <a href="{{ route('admin.rental-categories.show', $rentalCategory) }}" 
    ...
    <!-- Formulaire -->
    <form method="POST" action="{{ route('admin.rental-categories.update', $rentalCategory) }}" class="space-y-6">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   id="name" 
                                   name="name" 
                                   value="{{ old('name', $rentalCategory->name) }}"
                                   required
                        ...
                        <div>
                            ...
                            <textarea id="description" 
                                      name="description" 
                                      rows="4"
                                      class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-300 @enderror">{{ old('description', $rentalCategory->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   id="meta_title" 
                                   name="meta_title" 
                                   value="{{ old('meta_title', $rentalCategory->meta_title) }}"
                                   maxlength="255"
                        ...
                        <div>
                            ...
                            <textarea id="meta_description" 
                                      ...
                                      rows="3"
                                      maxlength="500"
                                      class="w-full border border-gray-300 rounded-md px-3 py-2 text-sm focus:ring-indigo-500 focus:border-indigo-500 @error('meta_description') border-red-300 @enderror">{{ old('meta_description', $rentalCategory->meta_description) }}</textarea>
                            <div class="mt-1 flex justify-between text-xs text-gray-500">
                    ...
                    <div class="space-y-4">
                        ...
                        <div>
                            ...
                            <input type="text" 
                                   id="icon" 
                                   name="icon" 
                                   value="{{ old('icon', $rentalCategory->icon) }}"
                        ...
                        <div>
                            ...
                            <input type="number" 
                                   id="display_order" 
                                   name="display_order" 
                                   value="{{ old('display_order', $rentalCategory->display_order ?? 0) }}"
                                   min="0"
                        ...
                        <div>
                            <div class="flex items-center">
                                <input type="checkbox" 
                                       ...
                                       name="is_active" 
                                       value="1"
                                       {{ old('is_active', $rentalCategory->is_active) ? 'checked' : '' }}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function updatePreview() {
        const title = metaTitleInput.value || nameInput.value || '{{ $rentalCategory->name }}';
        const description = metaDescriptionInput.value || descriptionInput.value || '{{ Str::limit($rentalCategory->description, 160) }}';
    ...
    function updateCharCount() {
        ...
        if (metaTitleLength > 60 || (metaTitleLength > 0 && metaTitleLength < 30)) {
            ...
            metaTitleCount.className = 'text-xs text-gray-500';
        }
        
        if (metaDescriptionLength > 160 || (metaDescriptionLength > 0 && metaDescriptionLength < 120)) {
            ...
            metaDescriptionCount.className = 'text-xs text-gray-500';
        }
    }
