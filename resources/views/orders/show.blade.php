...
@section('title', __('app.orders.order_number', ['number' => $order->order_number]) . ' - FarmShop')
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                    ...
                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium
                        ...
                        @switch($order->status)
                            @case('pending')
                                ...
                                @break
                            @case('confirmed')
                                🔵 {{ __('app.status.confirmed') }}
                                @break
                            @case('shipped')
                                🟣 {{ __('app.status.shipped') }}
                                @break
                            @case('delivered')
                                🟢 {{ __('app.status.delivered') }}
                                @break
                            @case('cancelled')
                                🔴 {{ __('app.status.cancelled') }}
                                @break
                            @case('return_requested')
                                🔶 {{ __('app.status.return_requested') }}
                                @break
                            @case('returned')
        ...
        <div class="grid lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white rounded-lg shadow p-6">
                    <h2 class="text-lg font-semibold text-gray-900 mb-4">{{ __('app.orders.delivery_address') }}</h2>
                    <div class="text-sm">
                            ...
                            <span class="capitalize">
                                @switch($order->payment_method)
                                    @case('card')
                                        ...
                                        @break
                                    @case('paypal')
                                        {{ __('app.orders.payment_methods.paypal') }}
                                        @break
                                    @case('bank_transfer')
                                        {{ __('app.orders.payment_methods.bank_transfer') }}
                                        @break
                                    @default
                        ...
                        <div class="flex justify-between text-sm">
                            ...
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium
                                ...
                                @switch($order->payment_status)
                                    @case('paid')
                                        {{ __('app.orders.payment_statuses.paid') }}
                                        @break
                                    @case('pending')
                                        {{ __('app.orders.payment_statuses.pending') }}
                                        @break
                                    @case('failed')
                                        {{ __('app.orders.payment_statuses.failed') }}
                                        @break
                                    @default
            ...
            <form id="returnForm" method="POST" action="">
                ...
                <div class="mb-4">
                    <label for="return_reason" class="block text-sm font-medium text-gray-700 mb-2">
                        {{ __('app.orders.return_reason_label') }}
                    </label>
        ...
        fetch(`/api/orders/${orderData.order.id}/status`)
            ...
            .then(data => {
                if (data.status === 'success') {
                    ...
                    console.log('{{ __('app.orders.status_updated') }}', data.order.status);
                }
            })
            .catch(error => {
                console.log('{{ __('app.orders.update_error') }}', error);
            });
    }
}
