...
foreach ($tocItems as $item) {
    $section->addText($item[0] . str_repeat('.', 80 - strlen($item[0]) - strlen($item[1])) . $item[1], $contentStyle);
}
...
// F1 à F33 et A1 à A10 - Je vais créer toutes les exigences fonctionnelles
...
// Continue avec les autres exigences fonctionnelles F11-F33 et A1-A10...
// Pour économiser l'espace, je vais ajouter quelques exemples clés
...
$section->addTitle('Relations interactions', 3);
$section->addText('Un utilisateur peut envoyer plusieurs contacts - Un contact appartient a un utilisateur [1-*]', $contentStyle);
$section->addText('Un utilisateur peut aimer plusieurs produits - Un produit peut etre aime par plusieurs utilisateurs [*-*]', $contentStyle);
$section->addText('Un utilisateur peut avoir plusieurs produits en wishlist - Un produit peut etre dans plusieurs wishlists [*-*]', $contentStyle);
...
$section->addTitle('Navigation Tailwind avec emojis Unicode', 2);
$section->addText('La navigation principale utilise Tailwind CSS avec des composants Alpine.js. Chaque element de menu est accompagne d\'un emoji Unicode pour ameliorer la reconnaissance visuelle et l\'accessibilite.', $contentStyle);

$section->addTextBreak();

$section->addText('Structure de navigation implementee :', $contentStyle);
$section->addText('• Logo FarmShop : Emoji tracteur 🚜 + texte en farm-green-500', $contentStyle);
...
$section->addText('• Panier d\'achat : Icone 🛒 avec compteur dynamique', $contentStyle);
$section->addText('• Panier location : Icone 📅 avec badge info', $contentStyle);
$section->addText('• Wishlist : Icone ❤️ avec animation', $contentStyle);
...
$section->addText('L\'interface reflete la double vocation de FarmShop avec des composants visuels distincts implementes avec les nouvelles fonctionnalites Laravel 11. Les paniers d\'achat (farm-green) et de location (blue-500) utilisent des contrôleurs separes pour optimiser les performances.', $contentStyle);
...
$section->addText('• Accessibilite renforcee avec emojis Unicode et ARIA', $contentStyle);
...
$objWriter->save(__DIR__ . '/12_Rapport_Ecrit_Laravel11.docx');

echo "Rapport ecrit Laravel 11 mis a jour avec succes !\n";
echo "Emplacement : " . __DIR__ . "/12_Rapport_Ecrit_Laravel11.docx\n";
echo "Document academique professionnel avec mise a jour vers Laravel 11 LTS genere !\n";
